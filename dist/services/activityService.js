...
exports.getActivitiesByWallet = getActivitiesByWallet;
exports.saveActivitiesBatch = saveActivitiesBatch;
exports.checkForDeposits = checkForDeposits;
const Activity_1 = __importDefault(require("../models/Activity"));
const ethPriceService_1 = require("./ethPriceService");
 ...
 */
async function retryDbOperation(operation, maxAttempts = activityMonitor_1.ACTIVITY_MONITOR_CONFIG.DB_RETRY_MAX_ATTEMPTS, baseDelay = activityMonitor_1.ACTIVITY_MONITOR_CONFIG.DB_RETRY_DELAY_BASE) {
    let lastError;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            ...
            logger_1.logger.warn(`Database operation failed, retrying in ${delayMs}ms (attempt ${attempt + 1}/${maxAttempts})`, { error: error.message });
            await delay(delayMs);
        }
    }
...
async function saveActivity(activityData) {
    try {
        ...
        const existing = await Activity_1.default.findOne({
        ...
        if (activityData.amount && activityData.amount !== '0') {
            try {
                ...
                const amountInEth = parseFloat(ethers_1.ethers.formatEther(activityData.amount));
                ...
                if (!isFinite(amountInEth) || isNaN(amountInEth) || amountInEth < 0) {
                    logger_1.logger.warn('Invalid amountInEth calculated:', { amount: activityData.amount, amountInEth });
                    amountUSD = '0';
                }
                else if (!isFinite(ethPrice) || isNaN(ethPrice) || ethPrice <= 0) {
                    logger_1.logger.warn('Invalid ethPriceUSD:', { ethPriceUSD, ethPrice });
                    amountUSD = '0';
                }
                else {
                    ...
                    if (!isFinite(usdValue) || isNaN(usdValue) || usdValue < 0) {
                        logger_1.logger.warn('Invalid USD value calculated:', { amountInEth, ethPrice, usdValue });
                        amountUSD = '0';
                    }
                    else {
                ...
                logger_1.logger.error('Error calculating USD amount for activity', error);
                // Continue without USD amount
            }
        }
        // Create activity with retry logic
        const activity = await retryDbOperation(async () => {
            return await Activity_1.default.create({
                ...activityData,
                txHash: activityData.txHash.toLowerCase(),
 ...
 */
async function getActivitiesByWallet(walletAddress, chainId, page = 1, pageSize = 10, type) {
    ...
    const [activities, totalCount] = await Promise.all([
        Activity_1.default.find(query)
            .sort({ blockTimestamp: -1 }) // Newest first
            .skip(skip)
            .limit(limit)
            .lean(),
        Activity_1.default.countDocuments(query)
    ]);
 ...
 */
async function saveActivitiesBatch(activitiesData) {
    if (activitiesData.length === 0) {
        logger_1.logger.debug('saveActivitiesBatch called with empty array');
        return [];
    }
    logger_1.logger.info(`saveActivitiesBatch: Processing ${activitiesData.length} activities`, {
    ...
    try {
        ...
        logger_1.logger.debug(`saveActivitiesBatch: Checking for duplicates`, {
            txHashCount: txHashes.length,
            chainIds: chainIds
        });
        const existingActivities = await Activity_1.default.find({
            txHash: { $in: txHashes },
            chainId: { $in: chainIds }
        }).select('txHash chainId').lean();
        logger_1.logger.debug(`saveActivitiesBatch: Found ${existingActivities.length} existing activities`, {
            existingCount: existingActivities.length,
            existingTxHashes: existingActivities.map((a) => `${a.txHash}-${a.chainId}`)
        });
        const existingKeys = new Set(existingActivities.map(a => `${a.txHash.toLowerCase()}-${a.chainId}`));
        // Filter out duplicates
        const newActivities = activitiesData.filter(a => !existingKeys.has(`${a.txHash.toLowerCase()}-${a.chainId}`));
        if (newActivities.length === 0) {
            logger_1.logger.info(`saveActivitiesBatch: All ${activitiesData.length} activities were duplicates`, {
                totalCount: activitiesData.length,
                duplicateTxHashes: txHashes
            });
            return [];
        }
        logger_1.logger.info(`saveActivitiesBatch: ${newActivities.length} new activities to insert (${activitiesData.length - newActivities.length} duplicates filtered)`, {
        ...
        try {
            ...
            logger_1.logger.error('Error fetching ETH price for batch USD calculation', error);
        ...
        logger_1.logger.info(`saveActivitiesBatch: Attempting to insert ${activitiesToInsert.length} activities into database`, {
        ...
        const savedActivities = await retryDbOperation(async () => {
            return await Activity_1.default.insertMany(activitiesToInsert, {
                ordered: false // Continue inserting even if some fail (duplicates)
            });
        });
        logger_1.logger.info(`saveActivitiesBatch: Successfully inserted ${savedActivities.length} activities`, {
        ...
        logger_1.logger.error(`saveActivitiesBatch: Error occurred`, {
            error: error.message,
        ...
        if (error.code === 11000 || error.writeErrors) {
            logger_1.logger.info(`saveActivitiesBatch: Handling partial failure (duplicates)`, {
                errorCode: error.code,
            ...
            const txHashes = activitiesData.map(a => a.txHash.toLowerCase());
            const chainIds = [...new Set(activitiesData.map(a => a.chainId))];
            const inserted = await Activity_1.default.find({
                txHash: { $in: txHashes },
                chainId: { $in: chainIds }
            }).lean();
            logger_1.logger.info(`saveActivitiesBatch: Retrieved ${inserted.length} successfully inserted activities after partial failure`, {
 ...
 */
async function checkForDeposits(_walletAddress, _chainId, _fromBlock, _toBlock) {
