...
userSchema.methods.addWalletAddress = function(address: string, isSmartWallet: boolean = false): Promise<IUser> {
  const existingWallet = this.walletAddresses.find(
    (wallet: { address: string }) => wallet.address.toLowerCase() === address.toLowerCase()
  );
...
userSchema.methods.setPrimaryWallet = function(address: string): Promise<IUser> {
  this.walletAddresses.forEach((wallet: { address: string; isPrimary: boolean }) => {
...
userSchema.methods.verifyWallet = function(address: string): Promise<IUser> {
