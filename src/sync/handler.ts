  ...
  try {
    ...
    if (priceData && token) {
      ...
      if (!existingPrice) {
        ...
        const savedHistory = await TokenHistory.create(historyData);
        ...
        emitTokenPriceUpdate(priceData.tokenAddress?.toLowerCase() || '', {
  ...
  try {
    ...
    if (boughtEvents.length > 0) {
      ...
      for (const event of boughtEvents) {
        ...
        const tokenPrice = ethers.formatUnits(
          (ethers.toBigInt(decodedEvent.args[6]) * 10n ** 18n) / ethers.toBigInt(decodedEvent.args[7]),
          18
        );
