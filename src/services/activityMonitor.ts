...
import { saveActivity, saveActivitiesBatch } from './activityService';
import { getProvider, getConfiguredChains, getWsProvider, getWsUrl, wsConnectionAttempts, wsProviderCache } from '../config/blockchain';
import { logger } from '../utils/logger';
import { ACTIVITY_MONITOR_CONFIG } from '../config/activityMonitor';

// Store monitored wallets per chain
const monitoredWallets = new Map<number, Set<string>>();
...
const wsProviders = new Map<number, ethers.WebSocketProvider>();
const wsSubscriptions = new Map<number, { pending: any; block: any }>();
const wsHealthChecks = new Map<number, NodeJS.Timeout>(); // Track health check intervals
const wsTimeBasedIntervals = new Map<number, NodeJS.Timeout>(); // Track time-based check intervals (10-second intervals as fallback)
const wsBlockProcessingTimeouts = new Map<number, NodeJS.Timeout>(); // Track debounced block processing timeouts per chain

// Track last checked block per chain
const lastCheckedBlock = new Map<number, number>();

// Track last known block number per chain (from WebSocket events)
// This eliminates the need for getBlockNumber RPC calls
const lastKnownBlockNumber = new Map<number, number>();

// Track timestamp of last known block number (for staleness checks)
const lastKnownBlockTimestamp = new Map<number, number>(); // chainId -> timestamp

// Track blocks currently being processed to prevent duplicate fetches
const blocksBeingProcessed = new Map<number, Set<number>>(); // chainId -> Set<blockNumber>

// Rate limit tracking for adaptive throttling
const rateLimitCount = new Map<number, number>(); // Consecutive rate limit errors per chain
const rateLimitFrequency = new Map<number, number[]>(); // Timestamps of rate limit errors (for frequency calculation)

// Circuit breaker: track when to stop checking due to excessive rate limits
const circuitBreaker = new Map<number, { enabled: boolean; until: number }>();
...
const blocksSinceLastCheck = new Map<number, number>();
 ...
 */
function isWebSocketReady(chainId: number): boolean {
  const wsProvider = wsProviders.get(chainId);
  if (!wsProvider) return false;
  
  try {
    const underlyingWs = (wsProvider as any).websocket || (wsProvider as any)._websocket;
    if (underlyingWs) {
...
const globalRequestQueue: Array<{ resolve: () => void; timestamp: number }> = [];
...
async function waitForRequestSlot(): Promise<void> {
  return new Promise((resolve) => {
    ...
    const delayNeeded = Math.max(0, ACTIVITY_MONITOR_CONFIG.MIN_DELAY_BETWEEN_GET_BLOCK_NUMBER - timeSinceLastRequest);
    
    if (activeRequests < ACTIVITY_MONITOR_CONFIG.MAX_CONCURRENT_REQUESTS && delayNeeded === 0) {
      activeRequests++;
      ...
      if (activeRequests < ACTIVITY_MONITOR_CONFIG.MAX_CONCURRENT_REQUESTS) {
  ...
  if (globalRequestQueue.length > 0) {
    ...
    if (next) {
      ...
      const delayNeeded = Math.max(
        ACTIVITY_MONITOR_CONFIG.MIN_DELAY_BETWEEN_GET_BLOCK_NUMBER,
        ACTIVITY_MONITOR_CONFIG.MIN_DELAY_BETWEEN_GET_BLOCK_NUMBER - timeSinceLastRequest
      );
...
async function getBlockNumberWithRetry(
  ...
  chainId: number,
  maxRetries: number = ACTIVITY_MONITOR_CONFIG.MAX_RETRIES
): Promise<number> {
    ...
    try {
      if (attempt > 0) {
        const backoffDelay = Math.min(
          ACTIVITY_MONITOR_CONFIG.RETRY_BACKOFF_BASE * Math.pow(2, attempt - 1),
          ACTIVITY_MONITOR_CONFIG.RETRY_BACKOFF_MAX
        );
        logger.debug(`Retrying getBlockNumber for chain ${chainId}`, { attempt: attempt + 1, maxRetries, backoffDelay });
      ...
      const isRateLimitError = 
        ...
        (Array.isArray(error?.value) && error.value.some((v: any) => v.code === -32005 || v.code === 429));
      ...
      if (isRateLimitError) {
        ...
        if (errors.length > ACTIVITY_MONITOR_CONFIG.RATE_LIMIT_FREQUENCY_MAX_SIZE) {
          errors.shift();
        }
        ...
        if (attempt === maxRetries - 1) {
          logger.error(`Failed to get block number for chain ${chainId} after ${maxRetries} attempts`, error);
          throw error;
        }
        continue;
      } else {
 ...
 */
async function getBlockWithFullTransactions(
  provider: ethers.JsonRpcProvider,
  blockNum: number,
  chainId: number
  ...
  try {
    ...
    const rpcResponse: any = await provider.send('eth_getBlockByNumber', [blockHex, true]);
    
    if (!rpcResponse || !rpcResponse.transactions) {
      logger.debug(`RPC returned empty block or no transactions for block ${blockNum}`, {
        chainId,
        blockNum
      });
      return null;
    ...
    if (transactionsAreFull) {
      ...
      try {
        ...
        const blockNumber = ethers.getNumber(rpcResponse.number);
        const timestamp = ethers.getNumber(rpcResponse.timestamp);
        
        // Parse full transactions from RPC response into TransactionResponse-like objects
        const fullTransactions = rpcResponse.transactions.map((tx: any, index: number) => ({
          hash: tx.hash,
          to: tx.to,
          from: tx.from,
          value: tx.value ? ethers.getBigInt(tx.value) : 0n,
          data: tx.input || '0x',
          blockNumber: blockNumber,
          blockHash: rpcResponse.hash,
          transactionIndex: tx.transactionIndex !== undefined ? ethers.getNumber(tx.transactionIndex) : index,
          gasLimit: tx.gas ? ethers.getBigInt(tx.gas) : 0n,
          gasPrice: tx.gasPrice ? ethers.getBigInt(tx.gasPrice) : null,
          nonce: tx.nonce !== undefined ? ethers.getNumber(tx.nonce) : 0,
          type: tx.type !== undefined ? ethers.getNumber(tx.type) : 0,
          accessList: tx.accessList || null,
          maxFeePerGas: tx.maxFeePerGas ? ethers.getBigInt(tx.maxFeePerGas) : null,
          maxPriorityFeePerGas: tx.maxPriorityFeePerGas ? ethers.getBigInt(tx.maxPriorityFeePerGas) : null,
          chainId: tx.chainId ? ethers.getNumber(tx.chainId) : chainId,
          confirmations: 1,
        }));
        ...
        const block: any = {
          ...
          gasLimit: rpcResponse.gasLimit ? ethers.getBigInt(rpcResponse.gasLimit) : 0n,
          gasUsed: rpcResponse.gasUsed ? ethers.getBigInt(rpcResponse.gasUsed) : 0n,
          ...
          difficulty: rpcResponse.difficulty ? ethers.getBigInt(rpcResponse.difficulty) : null,
          totalDifficulty: rpcResponse.totalDifficulty ? ethers.getBigInt(rpcResponse.totalDifficulty) : null,
          nonce: rpcResponse.nonce || null,
          baseFeePerGas: rpcResponse.baseFeePerGas ? ethers.getBigInt(rpcResponse.baseFeePerGas) : null,
        };
        ...
        if (block.transactions.length > 0) {
          ...
          if (typeof firstBlockTx === 'string') {
            ...
            logger.warn(`Block ${blockNum} transactions are still hashes after parsing`, {
              chainId,
              blockNum
            });
          ...
          logger.debug(`Successfully fetched block ${blockNum} with ${block.transactions.length} full transactions via eth_getBlockByNumber(blockNum, true)`, {
            chainId,
            blockNum,
          ...
          logger.debug(`Block ${blockNum} has no transactions`, {
            chainId,
            blockNum
          });
          return block as ethers.Block;
        ...
        logger.warn(`Failed to parse RPC response for block ${blockNum}, using fallback`, {
          chainId,
          blockNum,
      ...
      logger.warn(`Ankr returned transaction hashes for block ${blockNum} despite eth_getBlockByNumber(blockNum, true)`, {
        chainId,
        blockNum,
    ...
    logger.debug(`Direct RPC call failed for block ${blockNum}, using fallback`, {
      chainId,
      blockNum,
 ...
 */
async function getAllEmbeddedWallets(): Promise<Array<{ address: string; chainId: number; userId: string }>> {
  try {
    const wallets: Array<{ address: string; chainId: number; userId: string }> = [];
    const chains = getConfiguredChains();
    
    const BATCH_SIZE = ACTIVITY_MONITOR_CONFIG.DB_BATCH_SIZE;
    let skip = 0;
    let hasMore = true;
    ...
    logger.error('Error fetching embedded wallets', error);
    return [];
  }
}
...
function processBlockTransactions(
  ...
  if (block.transactions.length > 0 && walletSet.size > 0) {
    logger.debug(`Processing ${block.transactions.length} transactions in block ${blockNum} on chain ${chainId}`, {
      chainId,
      blockNum,
      transactionCount: block.transactions.length,
  ...
  let sampleTxTos: string[] = []; // Collect sample addresses for analysis
  ...
  for (const tx of block.transactions) {
    ...
    if (txCount === 0 && skippedStrings === 0 && skippedContractCreation === 0) {
      ...
      logger.debug(`First transaction in block ${blockNum} analysis`, {
        chainId,
        blockNum,
        txType: typeof tx,
    ...
    if (txCount <= 5 && walletSet.size > 0) {
      ...
      logger.debug(`Checking transaction ${txCount} in block ${blockNum}`, {
        chainId,
        blockNum,
        txTo,
    ...
    const isValidTransfer = 
        isToMonitoredWallet && // Transaction is to a monitored wallet
    ...
    if (isToMonitoredWallet) {
      logger.debug(`Transaction to monitored wallet found in block ${blockNum}`, {
        chainId,
        blockNum,
    ...
    if (isToMonitoredWallet && !isValidTransfer) {
      logger.debug(`Transaction to monitored wallet but not detected as deposit`, {
        chainId,
        txHash: txHash || 'missing',
        txTo,
        txFrom: txFrom || 'missing',
    ...
    if (isValidTransfer) {
      const amountEth = ethers.formatEther(txValue);
      ...
      (async () => {
        try {
          ...
          logger.info(`✅ Deposit saved to database immediately`, {
            chainId,
            txHash,
            walletAddress: txTo,
            fromAddress: txFrom,
          ...
          try {
            const { emitDepositDetected, emitBalanceUpdate } = await import('../socket/updateEmitter');
            ...
            emitDepositDetected({
              ...
              amountFormatted: ethers.formatEther(savedActivity.amount),
            ...
            if (savedActivity.userId) {
              try {
                ...
                logger.info(`✅ Balance update sent to user ${savedActivity.userId}`, {
                  walletAddress: savedActivity.walletAddress,
                ...
                logger.warn(`Failed to fetch/emit balance update for ${savedActivity.walletAddress}`, {
            ...
            logger.warn(`Failed to emit deposit event for ${txHash}`, error);
          }
        } catch (error: any) {
          logger.error(`❌ Failed to save deposit immediately, will retry in batch`, {
            chainId,
            txHash,
            error: error.message,
  ...
  logger.debug(`Transaction processing summary for block ${blockNum} on chain ${chainId}`, {
    chainId,
    blockNum,
  ...
  if (block.transactions.length > 0 && walletSet.size > 0 && sampleTxTos.length > 0) {
    ...
    logger.debug(`Transaction address analysis for block ${blockNum} on chain ${chainId}`, {
      chainId,
      blockNum,
      totalTransactions: block.transactions.length,
 ...
 */
async function checkForDeposits(chainId: number, providedBlockNumber?: number): Promise<void> {
  ...
  if (existingCheck) {
    ...
    logger.info(`⏭️ Skipping checkForDeposits for chain ${chainId} - already running`, {
      chainId,
      providedBlockNumber,
  ...
  const checkPromise = (async () => {
    ...
    logger.info(`🚀 Starting checkForDeposits for chain ${chainId}`, {
      chainId,
      providedBlockNumber,
      timestamp: new Date().toISOString()
    });
    ...
    try {
      ...
      try {
        ...
        logger.error(`No provider configured for chain ${chainId}`, error);
        return;
      }
      ...
      if (!wallets || wallets.size === 0) {
        ...
        logger.warn(`No wallets being monitored on chain ${chainId} - skipping deposit check`, {
          chainId,
          providedBlockNumber,
      ...
      if (consecutiveErrors > ACTIVITY_MONITOR_CONFIG.CIRCUIT_BREAKER_THRESHOLD || 
          recentErrors > ACTIVITY_MONITOR_CONFIG.MAX_ERRORS_PER_MINUTE) {
        const breaker = circuitBreaker.get(chainId);
        const now = Date.now();
        
        if (!breaker || breaker.until < now) {
          const cooldownUntil = now + ACTIVITY_MONITOR_CONFIG.CIRCUIT_BREAKER_COOLDOWN;
          circuitBreaker.set(chainId, { enabled: true, until: cooldownUntil });
          logger.error(`Circuit breaker enabled for chain ${chainId}`, {
            consecutiveErrors,
            recentErrors,
            cooldownMinutes: ACTIVITY_MONITOR_CONFIG.CIRCUIT_BREAKER_COOLDOWN / 60000
          });
        }
      ...
      if (!providedBlockNumber && 
          (consecutiveErrors > ACTIVITY_MONITOR_CONFIG.HIGH_ERROR_THRESHOLD || 
           recentErrors > ACTIVITY_MONITOR_CONFIG.HIGH_ERROR_RATE_THRESHOLD)) {
        return;
      }

      if (!providedBlockNumber && recentErrors > 2) {
        const preDelay = Math.min(
          ACTIVITY_MONITOR_CONFIG.PRE_DELAY_MULTIPLIER * recentErrors,
          ACTIVITY_MONITOR_CONFIG.MAX_PRE_DELAY
        );
      ...
      if (providedBlockNumber !== undefined) {
        ...
        if (trackedBlock && blockAge < BLOCK_CACHE_MAX_AGE) {
          ...
          logger.debug(`Using cached block number for chain ${chainId}`, {
            chainId,
          ...
          const maxWait = (wsReady && !hasReceivedBlock) 
            ? ACTIVITY_MONITOR_CONFIG.WS_BLOCK_WAIT_LONG 
            : ACTIVITY_MONITOR_CONFIG.WS_BLOCK_WAIT_SHORT;
          
          let trackedBlock: number | undefined;
          const startWait = Date.now();
          
          while (!trackedBlock && (Date.now() - startWait) < maxWait) {
            await delay(ACTIVITY_MONITOR_CONFIG.WS_BLOCK_CHECK_INTERVAL);
      ...
      if (lastBlock && currentBlock <= lastBlock) {
        logger.debug(`No new blocks to check on chain ${chainId}`, {
          chainId,
          lastBlock,
          currentBlock,
      ...
      if (actualBlockRange > MAX_EXPECTED_GAP_BLOCKS) {
        logger.warn(`Large block gap detected for chain ${chainId}`, {
          chainId,
          lastBlock,
          currentBlock,
          fromBlock,
          ...
          expectedRange: ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_SIZE + ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_OVERLAP,
      ...
      } else if (actualBlockRange > ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_SIZE * 3) {
        // Log moderate gaps at debug level
        logger.debug(`Moderate block gap detected for chain ${chainId}`, {
          chainId,
          lastBlock,
          currentBlock,
          fromBlock,
      ...
      logger.debug(`Starting deposit check for chain ${chainId}`, {
        chainId,
        providedBlockNumber,
        lastBlock,
        currentBlock,
      ...
      logger.debug(`Processing deposits for chain ${chainId}`, {
        chainId,
      ...
      let maxBlocksToCheck: number = ACTIVITY_MONITOR_CONFIG.NORMAL_BLOCK_RANGE;
      let BATCH_PAUSE: number = ACTIVITY_MONITOR_CONFIG.NORMAL_BATCH_PAUSE;
      let CONCURRENT_BLOCKS = 2;

      if (recentErrors > ACTIVITY_MONITOR_CONFIG.AGGRESSIVE_THROTTLE_ERROR_COUNT) {
        maxBlocksToCheck = ACTIVITY_MONITOR_CONFIG.AGGRESSIVE_BLOCK_RANGE;
        BATCH_PAUSE = ACTIVITY_MONITOR_CONFIG.AGGRESSIVE_BATCH_PAUSE;
        CONCURRENT_BLOCKS = 1;
      } else if (recentErrors > ACTIVITY_MONITOR_CONFIG.MODERATE_THROTTLE_ERROR_COUNT) {
        maxBlocksToCheck = ACTIVITY_MONITOR_CONFIG.MODERATE_BLOCK_RANGE;
        BATCH_PAUSE = ACTIVITY_MONITOR_CONFIG.MODERATE_BATCH_PAUSE;
        CONCURRENT_BLOCKS = 2;
      }
      ...
      logger.debug(`Processing blocks for chain ${chainId}`, {
        chainId,
        fromBlock,
        toBlock,
        ...
        throttling: recentErrors > ACTIVITY_MONITOR_CONFIG.MODERATE_THROTTLE_ERROR_COUNT ? 'active' : 'normal'
      });
        ...
        try {
          const blockPromises = batch.map(async (blockNum) => {
            ...
            if (lastBlock && blockNum <= lastBlock) {
              ...
              logger.debug(`⏭️ Block ${blockNum} skipped - marked as already processed`, {
                chainId,
                blockNum,
            ...
            try {
              ...
              try {
                ...
                logger.debug(`getBlockWithFullTransactions failed for block ${blockNum}, trying fallback`, {
                  chainId,
                  blockNum,
                  error: error.message
                });
              ...
              if (typeof block.transactions[0] === 'string') {
                const transactionHashes = block.transactions as string[];
                ...
                logger.debug(`Block ${blockNum} contains transaction hashes, fetching full transaction objects`, {
                  chainId,
                  blockNum,
                ...
                const transactionPromises = transactionHashes.map(async (txHash) => {
                  ...
                  try {
                    ...
                    logger.error(`Failed to fetch transaction ${txHash} for block ${blockNum} on chain ${chainId}`, {
                      chainId,
                      blockNum,
                      txHash,
                      error: error.message || String(error)
                ...
                (block as any).transactions = validTransactions;
                
                logger.debug(`Fetched ${validTransactions.length}/${transactionHashes.length} full transaction objects for block ${blockNum}`, {
                  chainId,
                  blockNum,
          ...
          for (const { blockNum, block, error, skipped } of blockResults) {
            if (skipped) {
              ...
              const skipReason = (blockResults.find(r => r.blockNum === blockNum) as any)?.reason || 'unknown';
              logger.warn(`⏭️ Block ${blockNum} skipped on chain ${chainId}`, {
                chainId,
                blockNum,
                reason: skipReason,
                fromBlock,
            ...
            if (error) {
              ...
              if (isRateLimitError) {
                ...
                if (currentConsecutiveErrors > ACTIVITY_MONITOR_CONFIG.EXCESSIVE_ERROR_THRESHOLD) {
                  continue;
                }
                ...
                if (currentConsecutiveErrors + 1 > ACTIVITY_MONITOR_CONFIG.CIRCUIT_BREAKER_THRESHOLD || 
                    currentRecentErrors > ACTIVITY_MONITOR_CONFIG.MAX_ERRORS_PER_MINUTE) {
                  const cooldownUntil = Date.now() + ACTIVITY_MONITOR_CONFIG.CIRCUIT_BREAKER_COOLDOWN;
                  circuitBreaker.set(chainId, { enabled: true, until: cooldownUntil });
                  return;
                }
                ...
                if (isAlreadyProcessed || isBeingProcessedNow) {
                  logger.debug(`Skipping retry for block ${blockNum} - already processed or being processed`, {
                    chainId,
                    blockNum,
                    lastBlock,
                    isAlreadyProcessed,
                ...
                try {
                  ...
                  let retryBlock: ethers.Block | null = null;
                  try {
                    retryBlock = await getBlockWithFullTransactions(provider, blockNum, chainId);
                  } catch (error: any) {
                    // Fallback to normal getBlock
                    logger.debug(`getBlockWithFullTransactions failed in retry for block ${blockNum}`, {
                      chainId,
                      blockNum,
                      error: error.message
                    });
                  ...
                  if (retryBlock && retryBlock.transactions && retryBlock.transactions.length > 0 && typeof retryBlock.transactions[0] === 'string') {
                    const transactionHashes = retryBlock.transactions as string[];
                    const transactionPromises = transactionHashes.map(async (txHash) => {
                      await waitForRequestSlot();
                      try {
                        const tx = await provider.getTransaction(txHash);
                        ...
                        logger.error(`Failed to fetch transaction ${txHash} for retry block ${blockNum} on chain ${chainId}`, {
                          chainId,
                          blockNum,
                          txHash,
                          error: error.message || String(error)
                    ...
                    const transactions = await Promise.all(transactionPromises);
                    const validTransactions = transactions.filter(tx => tx !== null);
                    (retryBlock as any).transactions = validTransactions;
                  }
                  ...
                  logger.error(`Failed to retry block ${blockNum} on chain ${chainId}`, retryError);
            ...
            if (block && block.transactions && block.transactions.length > 0) {
              ...
              if (walletSet.size > 0) {
                logger.debug(`Processing block ${blockNum} on chain ${chainId}`, {
                  chainId,
                  blockNum,
                  transactionCount: block.transactions.length,
                  walletSetSize: walletSet.size
          ...
          logger.error(`Error processing batch on chain ${chainId}`, batchError);
          continue;
        }
      }

      // Log summary of block processing - CHANGED TO INFO LEVEL for visibility
      logger.info(`✅ Completed processing blocks for chain ${chainId}`, {
        chainId,
      ...
      logger.info(`🔍 Checking depositsToSave before save on chain ${chainId}`, {
        chainId,
      ...
      if (depositsToSave.length > 0) {
        logger.info(`Preparing to save ${depositsToSave.length} deposits on chain ${chainId}`, {
          chainId,
        ...
        try {
          ...
          logger.info(`Calling saveActivitiesBatch with ${activitiesToSave.length} activities on chain ${chainId}`, {
            chainId,
          ...
          const savedActivities = await saveActivitiesBatch(activitiesToSave);
          
          logger.info(`saveActivitiesBatch returned ${savedActivities.length} saved activities (requested ${activitiesToSave.length}) on chain ${chainId}`, {
            chainId,
          ...
          if (savedActivities.length > 0) {
            logger.info(`✅ Saved ${savedActivities.length} new deposits on chain ${chainId}`, {
              chainId,
            ...
            try {
              ...
              const walletsToUpdate = new Map<string, { userId: string; chainId: number }>();
              ...
              savedActivities.forEach(activity => {
                emitDepositDetected({
                  ...
                  amountFormatted: ethers.formatEther(activity.amount),
              ...
              if (walletsToUpdate.size > 0) {
                ...
                const balanceUpdatePromises = Array.from(walletsToUpdate.entries()).map(async ([walletKey, walletInfo]) => {
                  ...
                  try {
                    ...
                    logger.debug(`✅ Balance update sent for wallet ${walletAddress}`, {
                      userId: walletInfo.userId,
                      balance: balanceFormatted,
                      chainId: walletInfo.chainId
                    });
                  } catch (balanceError: any) {
                    logger.warn(`Failed to fetch/emit balance update for wallet ${walletAddress}`, {
                      error: balanceError.message,
                      chainId: walletInfo.chainId
                    });
                  }
                ...
                logger.info(`✅ Emitted balance updates for ${walletsToUpdate.size} unique wallets on chain ${chainId}`);
              }
            } catch (error: any) {
              // Don't fail deposit saving if event emission fails
              logger.warn(`Failed to emit deposit events on chain ${chainId}`, error);
            }
          } else if (depositsToSave.length > 0) {
            // All deposits were duplicates - change to info level so it's visible
            logger.info(`⚠️ All ${depositsToSave.length} deposits were duplicates on chain ${chainId}`, {
              chainId,
          ...
          logger.error(`❌ Error batch saving deposits on chain ${chainId}`, {
            chainId,
            error: error.message,
            errorCode: error.code,
        ...
        logger.debug(`No deposits to save for chain ${chainId}`, { chainId });
      }
      ...
      if (highestProcessedBlock >= fromBlock) {
        ...
        logger.info(`Updated lastCheckedBlock for chain ${chainId}`, {
          chainId,
          fromBlock,
          toBlock,
        ...
        logger.warn(`No blocks were successfully processed for chain ${chainId}`, {
          chainId,
          fromBlock,
          toBlock,
      ...
      if (finalConsecutiveErrors > 0) {
        rateLimitCount.set(chainId, Math.max(0, finalConsecutiveErrors - ACTIVITY_MONITOR_CONFIG.ERROR_COUNT_REDUCTION_ON_SUCCESS));
      }
      
      // Log when check completes successfully
      logger.info(`✅ checkForDeposits completed successfully for chain ${chainId}`, {
        chainId,
        providedBlockNumber,
        timestamp: new Date().toISOString()
      });
    } catch (error: any) {
      logger.error(`❌ Error checking for deposits on chain ${chainId}`, {
        chainId,
        providedBlockNumber,
        error: error.message,
        stack: error.stack,
 ...
 */
async function setupWebSocketMonitoring(chainId: number): Promise<void> {
  try {
    ...
    if (!wsProvider) {
      ...
      if (wsUrl) {
        logger.warn(`WebSocket provider creation failed for chain ${chainId} - using polling only`, {
          chainId,
        ...
        logger.debug(`No WebSocket URL configured for chain ${chainId} - using polling only`);
      }
      return;
    }
    ...
    const wallets = monitoredWallets.get(chainId);
    if (!wallets || wallets.size === 0) {
      logger.debug(`No wallets to monitor on chain ${chainId} - skipping WebSocket setup`);
      return;
    }

    // Check if WebSocket is already set up
    if (wsProviders.has(chainId)) {
      logger.debug(`WebSocket monitoring already set up for chain ${chainId}`);
      return;
    }
    ...
    logger.info(`Setting up WebSocket monitoring for chain ${chainId}`, {
      chainId,
      walletCount: wallets.size
    });
    ...
    const blockHandler = async (blockNumber: number) => {
      try {
        ...
        const timeout = setTimeout(async () => {
          try {
            ...
            if (currentBlock !== undefined && (lastBlock === undefined || currentBlock > lastBlock)) {
              logger.debug(`Immediate deposit check triggered for chain ${chainId} (debounced)`, {
                chainId,
                lastBlock,
                currentBlock,
            ...
            logger.error(`Error in immediate deposit check for chain ${chainId}`, error);
          } finally {
        ...
        logger.error(`Error handling new block ${blockNumber}`, error);
      }
    };
    ...
    const timeBasedInterval = setInterval(() => {
      try {
        ...
        // Only process if we have new blocks to check
        if (currentBlock !== undefined && (lastBlock === undefined || currentBlock > lastBlock)) {
          logger.debug(`Time-based deposit check triggered for chain ${chainId}`, {
            chainId,
            lastBlock,
            currentBlock,
            blocksToProcess: lastBlock !== undefined ? currentBlock - lastBlock : 'initial',
          ...
          checkForDeposits(chainId, currentBlock).catch(error => {
            logger.error(`Error in time-based deposit check for chain ${chainId}`, error);
          });
        } else {
          logger.debug(`Skipping time-based check for chain ${chainId} - no new blocks`, {
            chainId,
            lastBlock,
            currentBlock
          });
        ...
        logger.error(`Error in time-based interval for chain ${chainId}`, error);
      }
    }, ACTIVITY_MONITOR_CONFIG.CHECK_INTERVAL_SECONDS * 1000);
    ...
    wsProvider.on('error', (error: any) => {
      ...
      if (errorMessage.includes('429') || errorMessage.includes('rate limit')) {
        logger.warn(`WebSocket 429 error for chain ${chainId} - will retry after backoff period (handled by getWsProvider)`);
      ...
      logger.error(`WebSocket error on chain ${chainId}`, error);
      wsProviders.delete(chainId);
      const healthCheck = wsHealthChecks.get(chainId);
      if (healthCheck) {
        clearInterval(healthCheck);
    ...
    if (underlyingWs && typeof underlyingWs.on === 'function') {
      underlyingWs.on('close', (code: number) => {
        logger.warn(`WebSocket closed for chain ${chainId} (code: ${code}) - will attempt to reconnect`);
        wsProviders.delete(chainId);
        const healthCheck = wsHealthChecks.get(chainId);
        if (healthCheck) {
          clearInterval(healthCheck);
        ...
        if (code !== 1000) { // 1000 = normal closure
          ...
          if (isInBackoff) {
            ...
            logger.debug(`WebSocket reconnection for chain ${chainId} delayed due to active backoff. Will retry in ${Math.ceil(reconnectDelay / 1000)}s`, {
              chainId,
      ...
      underlyingWs.on('error', (error: any) => {
        ...
        logger.error(`Underlying WebSocket error on chain ${chainId}`, error);
      });
    }
    ...
    logger.info(`WebSocket monitoring successfully set up for chain ${chainId}`, {
      chainId,
      walletCount: wallets.size
    });

    const waitForFirstBlock = new Promise<void>((resolve) => {
          ...
          if (lastKnownBlockNumber.has(chainId)) {
            ...
            if (lastBlock !== undefined && currentBlock > lastBlock) {
              ...
              if (gap > MAX_EXPECTED_GAP_BLOCKS) {
                ...
                const syncBlock = Math.max(
                  currentBlock - ACTIVITY_MONITOR_CONFIG.INITIAL_BLOCK_RANGE,
                  lastBlock
                );
                
                lastCheckedBlock.set(chainId, syncBlock);
                
                logger.info(`Synced lastCheckedBlock after WebSocket reconnect`, {
                  chainId,
                  ...
                  maxExpectedGapBlocks: MAX_EXPECTED_GAP_BLOCKS,
                  note: `Gap exceeded ${MAX_EXPECTED_GAP_BLOCKS} blocks. Synced to process recent ${ACTIVITY_MONITOR_CONFIG.INITIAL_BLOCK_RANGE} blocks.`
                });
              } else if (gap > ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_SIZE * 3) {
                // Log smaller gaps for monitoring (but don't sync)
                logger.debug(`WebSocket reconnected with moderate block gap`, {
                  chainId,
                  lastBlock,
                  currentBlock,
                  gap,
      ...
      }, ACTIVITY_MONITOR_CONFIG.WS_CONNECTION_CHECK_INTERVAL);
      
      setTimeout(() => {
        clearInterval(checkConnection);
        resolve();
      }, ACTIVITY_MONITOR_CONFIG.WS_FIRST_BLOCK_TIMEOUT);
    });

    await waitForFirstBlock;
  } catch (error: any) {
    logger.error(`Error setting up WebSocket monitoring for chain ${chainId}`, error);
  }
}
...
export async function startActivityMonitoring(): Promise<void> {
  try {
    ...
    logger.info('Loading embedded wallets for monitoring', {
      totalWallets: wallets.length,
      walletsByChain: Array.from(
    ...
    if (wallets.length === 0) {
      logger.info('No embedded wallets found to monitor');
      return;
    }

    // Group wallets by chain
    const walletsByChain = new Map<number, Array<{ address: string; userId: string }>>();
    ...
    walletsByChain.forEach((walletList, chainId) => {
      ...
      logger.info(`Wallets loaded for chain ${chainId}`, {
        chainId,
      ...
      const interval = setInterval(() => {
        ...
        if (wsReady && hasRecentBlock) {
          logger.debug(`Skipping polling for chain ${chainId} - WebSocket is active`, {
            chainId,
            lastKnownBlock,
            wsReady
          });
        ...
        checkForDeposits(chainId).catch(error => {
          logger.error(`Error in deposit check interval for chain ${chainId}`, error);
        });
      }, ACTIVITY_MONITOR_CONFIG.POLLING_INTERVAL);
      ...
      const initialDelay = ACTIVITY_MONITOR_CONFIG.INITIAL_CHECK_DELAY + (chainIndex * ACTIVITY_MONITOR_CONFIG.INITIAL_CHECK_STAGGER);
      setTimeout(() => {
        checkForDeposits(chainId).catch(error => {
          logger.error(`Error in initial deposit check for chain ${chainId}`, error);
        });
      }, initialDelay);
    ...
    logger.error('Error starting activity monitoring', error);
    throw error;
  }
}
  ...
  wsSubscriptions.forEach((subscriptions, chainId) => {
    ...
    if (wsProvider) {
      try {
        ...
        logger.error(`Error stopping WebSocket for chain ${chainId}`, error);
      }
    }
  });
 ...
 */
export async function addWalletToMonitoring(
  walletAddress: string,
  chainId: number,
  userId: string
): Promise<void> {
  try {
    ...
    if (monitoredWallets.get(chainId)!.has(walletLower)) {
      logger.debug(`Wallet ${walletLower} already being monitored on chain ${chainId}`, {
        chainId,
    ...
    logger.info(`Added wallet to monitoring`, {
      chainId,
      walletAddress: walletLower,
      userId,
    ...
    if (!isNowMonitored) {
      logger.error(`Failed to verify wallet was added to monitoring`, {
        chainId,
        walletAddress: walletLower,
        userId
      });
    ...
    if (!wsProvider) {
      ...
      setupWebSocketMonitoring(chainId).catch(error => {
        logger.warn(`Failed to set up WebSocket monitoring for chain ${chainId} after adding wallet`, error);
    ...
    setTimeout(async () => {
      try {
        ...
        try {
          ...
          logger.error(`No provider configured for chain ${chainId} in addWalletToMonitoring`, error);
          return;
        }
        ...
        if (trackedBlock) {
          ...
          if (trackedBlock) {
            ...
            try {
              ...
              logger.error(`Failed to get block number for chain ${chainId} in addWalletToMonitoring`, error);
              return;
            }
          }
        }
        ...
        const checkFromBlock = lastBlock 
          ? Math.max(lastBlock + 1, currentBlock - ACTIVITY_MONITOR_CONFIG.NEW_WALLET_BLOCK_RANGE)
          : Math.max(currentBlock - ACTIVITY_MONITOR_CONFIG.NEW_WALLET_BLOCK_RANGE, 0);
        const checkToBlock = currentBlock;
        
        // Skip if no new blocks to check
        if (checkFromBlock > checkToBlock) {
          logger.debug(`No new blocks to check for new wallet on chain ${chainId}`, {
            chainId,
            lastBlock,
            currentBlock,
            checkFromBlock,
        ...
        for (let blockNum = checkFromBlock; blockNum <= checkToBlock; blockNum++) {
          ...
          if (isAlreadyProcessed || isBeingProcessed) {
            logger.debug(`Skipping block ${blockNum} for new wallet - already processed or being processed`, {
              chainId,
              blockNum,
              isAlreadyProcessed,
              isBeingProcessed
          ...
          try {
            ...
            try {
              ...
              // Fallback to normal getBlock
              logger.debug(`getBlockWithFullTransactions failed in addWalletToMonitoring for block ${blockNum}`, {
                chainId,
                blockNum,
                error: error.message
              });
            ...
            if (block && block.transactions && block.transactions.length > 0 && typeof block.transactions[0] === 'string') {
              ...
              const transactionPromises = transactionHashes.map(async (txHash) => {
                ...
                try {
                  ...
                  logger.error(`Failed to fetch transaction ${txHash} for block ${blockNum} in addWalletToMonitoring on chain ${chainId}`, {
                    chainId,
                    blockNum,
                    txHash,
                    error: error.message || String(error)
            ...
            for (const tx of block.transactions) {
              ...
              if (txResponse.to.toLowerCase() === walletLower &&
                ...
                try {
                  ...
                  logger.info(`✅ Deposit detected for new wallet`, {
                    chainId,
                  ...
                  if (saveError.code !== 11000) {
                    logger.error(`Error saving deposit for new wallet`, saveError);
                  }
                }
              }
            }
        ...
        logger.error(`Error checking recent deposits for new wallet ${walletAddress}`, error);
      }
    }, ACTIVITY_MONITOR_CONFIG.NEW_WALLET_CHECK_DELAY);
  } catch (error) {
 ...
 */
export function getMonitoringDiagnostics(): {
  ...
  circuitBreakers: Record<number, { enabled: boolean; until: number }>;
 ...
 */
export function isWalletMonitored(walletAddress: string, chainId: number): boolean {
  const wallets = monitoredWallets.get(chainId);
 ...
 */
export function getWalletMonitoringStatus(walletAddress: string, chainId: number): {
  isMonitored: boolean;
 ...
 */
export async function triggerDepositCheck(chainId: number): Promise<{
  success: boolean;
  message: string;
  ...
  try {
    ...
    const lastBlock = lastCheckedBlock.get(chainId) || currentBlock - ACTIVITY_MONITOR_CONFIG.INITIAL_BLOCK_RANGE;
    const blockRange = ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_SIZE + ACTIVITY_MONITOR_CONFIG.BLOCKS_BATCH_OVERLAP;
    ...
    logger.error(`Error triggering deposit check for chain ${chainId}`, error);
    return {
      success: false,
...
export async function checkWalletForDeposits(
  ...
  try {
    ...
    const _checkFromBlock = fromBlock || Math.max(currentBlock - ACTIVITY_MONITOR_CONFIG.MANUAL_CHECK_BLOCK_RANGE, 0);
  } catch (error) {
